...
router.post('/employees/:id', async (req, res, next) => { try { await service.updateEmployee(req.session.user, req.params.id, req.body); flash(req,'success','تم تعديل بيانات الموظف'); res.redirect('/admin/employees'); } catch(e){ next(e); } });
router.post('/job-titles', (req, res, next) => { try { service.createJobTitle(req.session.user, req.body); flash(req,'success','تمت إضافة المسمى الوظيفي وربطه بالدائرة'); res.redirect('/admin/employees'); } catch(e){ next(e); } });
...
router.post('/cycles/:id/update', (req, res, next) => { try { service.updateCycle(req.session.user, req.params.id, req.body); flash(req,'success','تم تعديل الدورة'); res.redirect('/admin/cycles'); } catch(e){ next(e); } });
...
router.post('/cycles/:id/close', (req, res, next) => { try { service.closeCycle(req.session.user, req.params.id); flash(req,'success','تم إيقاف الدورة يدوياً'); res.redirect('/admin/cycles'); } catch(e){ next(e); } });
router.get('/templates', (req, res) => res.render('admin/templates', { title: 'النماذج', templates: service.templates(), lookups: service.lookups(), cycles: service.cycles(), employees: service.employees(), types: TemplateType }));
...
router.post('/templates/:id/update', (req, res, next) => { try { const items = normalizeItems(req.body); service.updateTemplate(req.session.user, req.params.id, { ...req.body, items }); flash(req,'success','تم تعديل النموذج'); res.redirect('/admin/templates'); } catch(e){ next(e); } });
router.post('/templates/:id/delete', (req, res, next) => { try { service.deleteTemplate(req.session.user, req.params.id); flash(req,'success','تم حذف النموذج'); res.redirect('/admin/templates'); } catch(e){ next(e); } });
