...
router.get('/', (req, res) => { const cycle = service.cycles().find(c=>c.isActive && c.isPublished); const targets = cycle ? service.availableTargets(req.session.user, cycle.id) : []; res.render('evaluations/index', { title: 'تقييماتي', cycle, targets }); });
